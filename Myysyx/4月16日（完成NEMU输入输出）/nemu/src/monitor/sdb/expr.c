...
/* We use the POSIX regex functions to process regular expressions.我们使用 POSIX 正则表达式函数来处理正则表达式。
 * Type 'man regex' for more information about POSIX regex functions.输入'man regex'命令可以查看POSIX 正则表达式函数的更多信息。
 */
#include <regex.h>
    ...
    for (i = 0; i < NR_REGEX; i ++) {
      ...
      if (regexec(&re[i], e + position, 1, &pmatch, 0) == 0 && pmatch.rm_so == 0) {
        ...
        switch (rules[i].token_type) {
          ...
          case TK_REG:
            //处理寄存器名称
            if (substr_len <= MUXDEF(CONFIG_RVE, 16, 32)) {
              strncpy(tokens[nr_token].str, substr_start, substr_len);
          ...
          case TK_BITWISE_NOT:
            ...
            break;
          default:
            panic("Unknown token type: %d", rules[i].token_type);//未知类型
        }
        break;
      }
    }
...
static int compute(int a, int b, int op) {
  switch (op) {
    ...
    case DEREF:
        ...
        //如果是虚拟地址
        word_t value = vaddr_read(a, sizeof(word_t));
                          return value; //解引用
