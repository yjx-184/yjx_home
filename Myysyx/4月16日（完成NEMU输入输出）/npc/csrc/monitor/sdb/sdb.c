...
static int cmd_info(char *args) {
  if (args == NULL) {
  #if CONFIG_CN
    printf("Error:'info' 命令需要后跟子命令('r' 或 'w')\n");
  #else
  ...
  #if CONFIG_CN
    printf("未知的子命令 '%s' 对于 'info'。支持的子命令: 'r' 和 'w'。\n", args);
  #else
...
static int cmd_w(char *args) {
  ...
  uint32_t value = expr(args, &success);  //调用expr函数进行求值
  if (success) {
...
static int cmd_b(char *args) {
  ...
  bool success;
  uint32_t addr = expr(args, &success); //解析地址表达式
  if (!success) {
  #if CONFIG_CN
    printf("无效的地址: %s\n", args);
  #else
...
static struct {
  ...
  { "si", "执行 n 条指令(默认:1)", cmd_si },

  { "info", "打印程序状态。使用 'r' 表示寄存器，使用 'w' 表示观察点", cmd_info },

  { "x", "扫描内存。格式: x N EXPR,其中 N 是要读取的 4 字节单元的数量,EXPR 是十六进制的起始地址", cmd_x },

  { "p", "求值表达式,格式: p EXPR", cmd_p },

  { "w", "设置监视点,格式: w EXPR", cmd_w },

  { "d", "删除监视点。用法: d N", cmd_d },
