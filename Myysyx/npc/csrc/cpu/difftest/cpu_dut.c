...
void init_difftest(char *ref_so_file, long img_size, int port) {
  ...
  printf("动态库文件 %s 打开成功。\n", ref_so_file);
  #else
  printf("Successfully loaded dynamic library file: %s\n", ref_so_file);
  #endif

  // 获取参考模型中的函数指针
  ref_difftest_memcpy = (void (*)(uint32_t, void*, size_t, bool))dlsym(handle, "difftest_memcpy");
  assert(ref_difftest_memcpy);
  // printf("成功获取参考模型中的 difftest_memcpy 函数指针。\n");

  ref_difftest_regcpy = (void (*)(void*, bool))dlsym(handle, "difftest_regcpy");
  assert(ref_difftest_regcpy);
  // printf("成功获取参考模型中的 difftest_regcpy 函数指针。\n");

  ref_difftest_exec = (void (*)(uint64_t))dlsym(handle, "difftest_exec");
  assert(ref_difftest_exec);
  // printf("成功获取参考模型中的 difftest_exec 函数指针。\n");

  ref_difftest_raise_intr = (void (*)(uint64_t))dlsym(handle, "difftest_raise_intr");
  assert(ref_difftest_raise_intr);
  // printf("成功获取参考模型中的 difftest_raise_intr 函数指针。\n");

  // 初始化参考模型
  void (*ref_difftest_init)(int) = (void (*)(int))dlsym(handle, "difftest_init");
  assert(ref_difftest_init);
#if CONFIG_CN
  Log("差分测试状态(Differential testing):%s", ANSI_FMT("ON", ANSI_FG_GREEN));
  Log("每条指令的执行结果都将与参考模型(%s)进行对比。"
      "这将极大有助于调试，但也会显著降低模拟性能。"
      "如无必要，可在 menuconfig 中关闭此功能。", ref_so_file);
#else
...
static void checkregs(CPU_state *ref, vaddr_t pc) {
  if (!isa_difftest_checkregs(ref, pc)) { // 调用ISA相关的寄存器检查
    npc_state.state = NPC_ABORT;          // 设置模拟器异常状态
    npc_state.halt_pc = pc;               // 记录停止时的PC值
...
void difftest_step(vaddr_t pc, vaddr_t npc) {
  ...
  if (skip_dut_nr_inst > 0) {
    ...
    if (skip_dut_nr_inst == 0)
    #if CONFIG_CN
      Log("NEMU 无法追上参考模型,ref.pc = " FMT_WORD "，当前 pc = " FMT_WORD, ref_r.pc, pc);
    #else
