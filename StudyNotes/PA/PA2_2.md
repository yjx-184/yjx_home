...
3. 在inst.c文件中开始处理链表内容。首先需要获取二进制的指令(isa-def.h中有定义指令解码信息的结构体，其中就包含了存储当前指令的二进制编码)。然后再把指令解析出来讲义提示到会用到jal和jalr。同时看了视频了解到，call和ret也都属于是伪指令。call等价于jalr x1, offset(x1)同时也有auipc+jalr的组合.ret等价于jalr x0, 0(x1)。同时还需要注意尾调用jalr x0, offset[11:0](x6)。  
![](./pic/函数跳转和返回指令.png)  
