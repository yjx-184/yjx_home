...
***  
完成过程：  
1. 命令行需要扩展，以至于能够添加elf文件。
2. 在传入elf文件后，需要打开并解析，通过链表操作获取符号表和字符串表内容。
   * 打开文件并读取 ELF 头，同时需要验证
   * 读取节头表并找到字符串表
   * 遍历节头表，查找符号表和字符串表
   * 解析符号表（.symtab）
   * 解析字符串表（.strtab）
...
用于 从文件读取数据.  
* void *ptr：指向数据存储缓冲区的指针，fread 读取的数据会存到这里。
* size_t size： 单个数据块的大小（以字节为单位）。
* size_t nmemb： 要读取的元素数量。
* FILE *stream： 输入流，即要读取的文件指针（通常由 fopen 获取）。

**fseek函数**:```int fseek(FILE *stream, long offset, int whence);```  
用于在文件流中 移动文件指针，从而实现 随机访问（不是顺序读取）。  
* FILE *stream：文件指针，由 fopen 返回。
* long offset： 偏移量，以字节为单位。可以是正数（向前移动）或负数（向后移动）。
* int whence：基准位置，决定 offset 计算的起点：
  * SEEK_SET	从文件开头开始偏移
...
格式化输出到文件.
* 标准输出 (stdout)：用于程序的正常输出，例如调试信息、结果等。
* 标准错误 (stderr)：用于输出错误信息、警告或其他需要特别关注的消息。
通过将错误信息输出到 stderr，可以将正常输出和错误信息分开处理，便于调试和日志管理。  

**ftell()**:```long ftell(FILE *stream);```  
用于获取文件流当前的读取/写入位置。

**解析ELF文件成功：**   
后面在cpu-exec.c我用了一个获取链表的函数，确保从elf文件解析后，链表接受到了内容。 
```
yjx@yjx-Lenovo-Legion-R7000-2020:~/ysyx-workbench/am-kernels/tests/cpu-tests$ make ARCH=riscv32-nemu ALL=recursion run
# 正在构建 recursion-run [riscv32-nemu]
# 正在构建 am-archive [riscv32-nemu]
# 正在构建 klib-archive [riscv32-nemu]
+ OBJCOPY -> build/recursion-riscv32-nemu.bin
mainargs=
+ CC src/monitor/monitor.c
+ LD /home/yjx/ysyx-workbench/nemu/build/riscv32-nemu-interpreter
[src/utils/log.c:30 init_log] Log is written to /home/yjx/ysyx-workbench/am-kernels/tests/cpu-tests/build/nemu-log.txt
[src/memory/paddr.c:76 init_mem] physical memory area [0x80000000, 0x87ffffff]
[src/monitor/monitor.c:79 load_img] The image is /home/yjx/ysyx-workbench/am-kernels/tests/cpu-tests/build/recursion-riscv32-nemu.bin, size = 728
成功打开 ELF 文件: /home/yjx/ysyx-workbench/am-kernels/tests/cpu-tests/build/recursion-riscv32-nemu.elf
ELF 头信息读取成功: 入口地址 0x80000000, 段头表偏移  0x171c, 段头表项个数 11
找到符号表: 在文件中的偏移 0x131c, 大小 736, 每项大小 16
找到字符串表: 在文件中的偏移 0x15fc, 大小 193
字符串表读取成功
符号表中共有 46 个符号
发现函数符号: 地址 0x80000254, 名称 _trm_init
发现函数符号: 地址 0x80000108, 名称 f3
发现函数符号: 地址 0x800001b0, 名称 check
发现函数符号: 地址 0x800000a4, 名称 f2
发现函数符号: 地址 0x80000000, 名称 _start
发现函数符号: 地址 0x800001c8, 名称 main
发现函数符号: 地址 0x80000010, 名称 f0
发现函数符号: 地址 0x8000005c, 名称 f1
发现函数符号: 地址 0x80000248, 名称 halt
[src/monitor/monitor.c:30 welcome] Trace: ON
...
[src/monitor/monitor.c:34 welcome] Build time: 15:15:42, Mar 19 2025
Welcome to riscv32-NEMU!
For help, type "help"
ELF符号表内容:
符号名: halt, 地址: 0x80000248
符号名: f1, 地址: 0x8000005c
符号名: f0, 地址: 0x80000010
符号名: main, 地址: 0x800001c8
符号名: _start, 地址: 0x80000000
符号名: f2, 地址: 0x800000a4
符号名: check, 地址: 0x800001b0
符号名: f3, 地址: 0x80000108
符号名: _trm_init, 地址: 0x80000254
[src/cpu/cpu-exec.c:289 cpu_exec] nemu: HIT GOOD TRAP at pc = 0x8000026c
[src/cpu/cpu-exec.c:240 statistic] host time spent = 23,964 us
[src/cpu/cpu-exec.c:241 statistic] total guest instructions = 4,545
[src/cpu/cpu-exec.c:242 statistic] simulation frequency = 189,659 inst/s
test list [1 item(s)]: recursion
[     recursion] PASS

```  

3. 在cpu-exec.c文件中开始处理链表内容。首先需要获取二进制的指令(isa-def.h中有定义指令解码信息的结构体，其中就包含了存储当前指令的二进制编码)。然后再把指令解析出来讲义提示到会用到jal和jalr。同时看了视频了解到，call和ret也都属于是伪指令。call等价于jalr x1, offset(x1)同时也有auipc+jalr的组合.ret等价于jalr x0, 0(x1)。同时还需要注意尾调用jalr x0, offset[11:0](x6)。  
![](./pic/函数跳转和返回指令.png)  
4. 在知道需要哪些指令后，我需要明确当遇到call和ret时都需要做什么。使用栈的结构来确认函数的嵌套程度。call需要根据地址来匹配函数名。同时找到目标函数后，打印输出。将该函数的结构体压入栈中，统计嵌套的深度。ret则是做弹出操作，然后打印内容。
5. 然后我在做的过程中（在进行解码inst时），这个过程是不是可以在之前inst.c文件中完成，因为其中有很多的宏定义可以帮助我减少很多操作，同时提高正确率。但是，当时并没有马上停止操作，而是实现完后，才敢重新操作，再看了头文件以后，我肯定我的想法是可以的！一顿复制重构后，实验结果也是按照预期实现，害...
...
**遇到的错误或困难：**  
1. 传入elf文件时，提示报错
   * 原因： 在```while ( (o = getopt_long(argc, argv, "-bhl:d:p:", table, NULL)) != -1)```中,没有加入e。（虽然写了匹配规则）
   * 解决： 弹出提示报错：可执行文件 /home/yjx/ysyx-workbench/nemu/build/riscv32-nemu-interpreter 不支持 -e 参数，但 Makefile 里 ARGS="... -e $(IMAGE).elf" 传入了 -e 选项，导致 NEMU 报错。getopt_long() 是 C 语言的一个标准库函数，用于解析命令行参数。所以我需要补上"e:"
2. 没有正确传入elf文件
   * 原因： 虽然我修改了makefile文件，但是因为我在parse_args()函数中 ```while ( (o = getopt_long(argc, argv, "-bhl:d:p:e:", table, NULL)) != -1)```的e后面没有加':'，导致一直无法传入elf文件。
3. 在遍历节表，寻找符号表和字符串表时，文件指针（fp）位置被错误修改，导致后续读取节表项失败。  
出现问题的地方：  
```
 if (shdr.sh_type == SHT_SYMTAB) {
      ...
      if (fread(&strtab, sizeof(strtab), 1, fp) != 1) { // 读取字符串表的节头
      // 在这里移动了fp文件指针，在读取符号表关联的字符串表节头后，文件指针停留在字符串表节头的位置，而后又没有恢复fp原来的位置，导致后续节表项读取失败。
      }
    }

```  
4. 在看完视频了解了函数的调用后，我关注到call伪指令也还和auipc有关系，我当时困惑的点在于，我是不是需要检查jalr上一个是不是auipc然后进行处理。后来我发现这是多此一举的事情。因为正常来说，在指令的执行过程中如果真遇到了auipc+jalr的组合指令，那么auipc其实已经完成了，而我们只需要处理jalr就可以了。我只需要处理是否发生了函数调用就可以了。  
...
***  
在尾调用优化中，如果一个函数调用处于“尾部位置”（即调用后不再有后续工作），编译器可以选择重用当前函数的栈帧而不必为被调用函数分配一个新的栈帧。这就会导致一些看上去不匹配的返回地址现象。  
...
以上就是init_difftest()函数完成之后，, DUT和REF就处于相同的状态了。  

接下来就可以进行逐条指令执行后的状态对比了, 实现这一功能的是difftest_step()函数(在nemu/src/cpu/difftest/dut.c中定义). 它会在cpu_exec()的主循环中被调用, 在NEMU中执行完一条指令后, 就在difftest_step()中让REF执行相同的指令(trace_and_difftest()函数会调用difftest_step())。  

然后读出REF中的寄存器, 并进行对比. 由于不同ISA的寄存器有所不同, 框架代码把寄存器对比抽象成一个ISA相关的API, 即isa_difftest_checkregs()函数(在nemu/src/isa/$ISA/difftest/dut.c中定义).   

根据讲义，isa_difftest_checkregs()是需要实现的，讲义这样描述： 把通用寄存器和PC与从DUT中读出的寄存器的值进行比较. 若对比结果一致, 函数返回true; 如果发现值不一样, 函数返回false, 框架代码会自动停止客户程序的运行. 特别地, isa_difftest_checkregs()对比结果不一致时, 第二个参数pc应指向导致对比结果不一致的指令, 可用于打印提示信息.  

***  
现在difftest的完程度已经很高了（似乎），只需要实现isa_difftest_checkregs()就可以了。

因此我们需要获得寄存器的值和pc值，通过以前读代码，在 NEMU ISA相关的API说明文档中，我们了解到寄存器相关的结构体内容，并且知道 寄存器结构的全局定义(```CPU_state cpu;```)。  

在reg.h中，也提供了获取通用寄存器值的宏```#define gpr(idx) (cpu.gpr[check_reg_idx(idx)])``` 。

读代码可知，在src/cpu/difftest/dut.c的difftest_step函数中，可以知道参考模型寄存器状态 ```CPU_state ref_r``` 。  

所以，我们需要明确的对象有了, DUT的寄存器: cpu.gpr[i], 参考模型的寄存器:ref_r->gpr[i], DUT的PC: cpu.pc， 参考模型的PC:ref_r->pc。  

最后实现结果（我把rem指令故意写错）：  
```
Welcome to riscv32-NEMU!
For help, type "help"
DiffTest: Register mismatch at PC= 0x800000c4
Reg[14]: REF = 0x00000000, DUT = 0x80000000
$0 = 0x00000000
ra = 0x80000000
sp = 0x80008ff0
gp = 0x00000000
...
t6 = 0x00000000
pc = 0x800000c8
[src/cpu/cpu-exec.c:292 cpu_exec] nemu: ABORT at pc = 0x800000c4
[src/cpu/cpu-exec.c:247 statistic] host time spent = 239 us
[src/cpu/cpu-exec.c:248 statistic] total guest instructions = 43
[src/cpu/cpu-exec.c:249 statistic] simulation frequency = 179,916 inst/s
指令执行记录（最近 16 条）:
    0x80000088: 00 50 05 13 addi	a0, zero, 5
    0x8000008c: fe c0 00 93 addi	ra, zero, -0x14
    0x80000090: ff a0 05 93 addi	a1, zero, -6
    0x80000094: 02 b0 e7 33 rem	a4, ra, a1
    0x80000098: ff e0 03 93 addi	t2, zero, -2
    0x8000009c: 08 77 10 63 bne	a4, t2, 0x80
    0x800000a0: 00 60 05 13 addi	a0, zero, 6
    0x800000a4: 80 00 00 b7 lui	ra, 0x80000
    0x800000a8: 00 10 05 93 addi	a1, zero, 1
    0x800000ac: 02 b0 e7 33 rem	a4, ra, a1
